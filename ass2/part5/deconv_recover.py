# part5/deconv_recover.py
# Minimal-edit fix: use un-clipped noisy image for processing and estimate S_f from noiseless blurred image.
import os
import sys
import math
import gc
...
# ---------------------------
# Gaussian noise generator (returns unclipped for processing, and clipped for saving)
# ---------------------------
def gaussian_noise_for_target_psnr(img: np.ndarray, target_psnr: float, rng=None):
    """
    Returns (noisy_unclipped, noisy_clipped, actual_psnr_vs_original, sigma)
    We'll use noisy_unclipped for spectral estimation and deconvolution (no clipping),
    but save noisy_clipped for visualization / PSNR reporting if desired.
    """
    if rng is None:
    ...
    # small corrective rescale (improve exactness before clipping)
...
def estimate_Sf_from_blurred(blurred: np.ndarray, Hf: np.ndarray, smooth_ksize: int = 7, epsH: float = 1e-8):
    """
    Compute S_blurred = |B|^2/(MN). Then S_f_est = S_blurred / (|H|^2 + epsH).
    Smooth and clip to stabilize.
    """
    M, N = blurred.shape
...
def adaptive_wiener_using_blurred(g_proc: np.ndarray, Hf: np.ndarray, blurred: np.ndarray, sigma2: float,
                                  smooth_ksize: int = 7):
    """
    Build K_map = sigma2 / S_f_est where S_f_est computed from noiseless blurred image.
    Use robust clipping and force large K where H is tiny.
    Use g_proc (unclipped noisy image) for G.
    """
...
# --- Insert into part5/deconv_recover.py near adaptive functions ---
...
def tune_and_apply_adaptive(g_proc, Hf, blurred, f, sigma2,
                            ...
                            kmax=1e5):
    """
    Grid-search small parameters: smoothing kernel size (for Sf estimate)
    and blending beta between local Sf and global prior. Pick best by PSNR(rec,f).
    Returns best_rec, best_K_map, (best_params)
    """
    best_psnr = -1.0
    best_rec = None
    best_K = None
...
# ---------------------------
# Main pipeline (minimal edits to use unclipped noisy for processing and Sf from blurred)
# ---------------------------
def main():
    if len(sys.argv) != 3:
    ...
    for target in noise_levels:
        ...
        # SAVE the clipped noisy for display; use unclipped for processing
        save_gray(os.path.join(outdir, f'g_psnr{target}_actual{actual_psnr:.2f}.png'), noisy_clipped)
        g_proc = noisy_unclipped  # use this for spectral computations (do NOT clip)
        g_display = noisy_clipped  # used only for saving/PSNR reporting

        # bands for f & g_display
        ...
        # warn & debug if adaptive didn't help
